...
// MFC综合实验Set.cpp : CMFC综合实验Set 类的实现
//
...
// 代码生成在 2020年6月8日, 14:35

IMPLEMENT_DYNAMIC(CMFC综合实验Set, CRecordset)

CMFC综合实验Set::CMFC综合实验Set(CDatabase* pdb)
	: CRecordset(pdb)
{
	m_ID = 0;
	column1 = L"";
...
// 储为其他格式或使用其他的用户身份验证。
CString CMFC综合实验Set::GetDefaultConnect()
{
	return _T("DSN=Picture;DBQ=D:\\MFC\\picture.accdb;DriverId=25;FIL=MS Access;MaxBufferSize=2048;PageTimeout=5;UID=admin;");
}

CString CMFC综合实验Set::GetDefaultSQL()
{
	return _T("[pic]");
}

void CMFC综合实验Set::DoFieldExchange(CFieldExchange* pFX)
{
...
#ifdef _DEBUG
void CMFC综合实验Set::AssertValid() const
{
	CRecordset::AssertValid();
}

void CMFC综合实验Set::Dump(CDumpContext& dc) const
{
	CRecordset::Dump(dc);
}
#endif //_DEBUG
